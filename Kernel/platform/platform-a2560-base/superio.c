...
*/
/*
unsigned int UnReset_LPC( ) {
  unsigned int i;
  unsigned int j = 0;
  *GABE_CTRL_LPC = *GABE_CTRL_LPC | 0x00000100; // This will set the LPC_RSTn which is its normal operation state.
  for (i = 0; i< 1000; i++) 
    j = j + 4;  // Ignore this, this is possibly for gcc to not discard the whole loop because of optimization

  *GABE_CTRL_LPC = *GABE_CTRL_LPC & 0xFFFFFEFF; // This will set the LPC_RSTn which is its normal operation state.
  for (i = 0; i< 400000; i++) 
    j = j - 4; 
  // So let's Unreset it
  *GABE_CTRL_LPC = *GABE_CTRL_LPC | 0x00000100; // This will set the LPC_RSTn which is its normal operation state.
