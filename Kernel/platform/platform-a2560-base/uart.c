 ...
 */
short uart_status(short uart) {
    short status = 0;

    // Check if there is data to be read
    if (uart_has_bytes(uart)) {
        status |= CDEV_STAT_READABLE;
    }

    // Check if the FIFO is full
    if (uart_can_send(uart)) {
 ...
 */
short uart_open(short uart, const uint8_t * spec, short mode) {
 ...
 */
short uart_read(short uart, uint8_t * buffer, short size) {
    short i = 0, count = 0;
    for (i = 0; i < size; i++) {
        buffer[i] = uart_get(uart);
        count++;
    }
 ...
 */
short uart_readline(short uart, uint8_t * buffer, short size) {
    short i = 0, count = 0;
    for (i = 0; i < size; i++) {
        char c = uart_get(uart);
        if ((c == '\n') || (c == '\r')) {
            break;
        }
        buffer[i] = c;
 ...
 */
short uart_write_b(short uart, unsigned char c) {
    uart_put(uart, c);
    return 0;
}
 ...
 */
short uart_write(short uart, const uint8_t * buffer, short size) {
    int i;
    for (i = 0; i < size; i++) {
        uart_put(uart, buffer[i]);
    }
    return 0;
}
